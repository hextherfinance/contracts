...
contract HexSharesRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accHexSharesPerShare; // Accumulated tSHAREs per share, times 1e18. See below.
    ...
    uint256 public tSharePerSecond = 0.00172047 ether; // 55000 bshare / (370 days * 24h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "HexSharesRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "HexSharesRewardPool: existing pool?");
        }
    }
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accHexSharesPerShare = pool.accHexSharesPerShare;
        uint256 tokenSupply = pool.token.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            accHexSharesPerShare = accHexSharesPerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accHexSharesPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accHexSharesPerShare = pool.accHexSharesPerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accHexSharesPerShare).div(1e18).sub(user.rewardDebt);
            if (_pending > 0) {
                safeHexSharesTransfer(_sender, _pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accHexSharesPerShare).div(1e18);
    ...
    function safeHexSharesTransfer(address _to, uint256 _amount) internal {
