...
contract HextherGenesisRewardPoolWL {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. HEXTHER to distribute.
        uint256 lastRewardTime; // Last time that HEXTHER distribution occurs.
        uint256 accHextherPerShare; // Accumulated HEXTHER per share, times 1e18. See below.
    ...
    // The time when HEXTHER mining ends.
    uint256 public poolEndTime;

    // TESTNET
    // uint256 public bombPerSecond = 3.0555555 ether; // 11000 HEXTHER / (1h * 60min * 60s)
    // uint256 public runningTime = 24 hours; // 1 hours
    // uint256 public constant TOTAL_REWARDS = 11000 ether;
    // END TESTNET

    // MAINNET
    uint256 public bombPerSecond = 0.00347222 ether; // 300 HEXTHER / (24h * 60min * 60s)
    uint256 public runningTime = 30 minutes; // 30 minutes
    ...
    constructor(
        ...
        for (uint256 i = 0; i < count; ++i) {
            setWhitelist(whitelist[i], true);
        } 
    }

    modifier onlyOperator() {
        require(operator == msg.sender, "HextherGenesisPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "HextherGenesisPool: existing pool?");
        }
    }
    ...
    // View function to see pending HEXTHER on frontend.
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        require (_whitelist[msg.sender] == true, "not WL") ;
    ...
    // Safe HEXTHER transfer function, just in case if rounding error causes pool to not have enough HEXTHERs.
    ...
    function setWhitelist(address _account, bool _isWL) public onlyOperator {
        _whitelist[_account] = _isWL;
    }
    ...
    function governanceRecoverUnsupported(
        ...
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (HEXTHER or lps) if less than 90 days after pool ends
