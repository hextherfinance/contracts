...
*/
contract Hexther is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the Hexther ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Hexther", "HEX") {
        // Mints 1 Hexther to contract creator for initial pool setup
    ...
    function _getHextherPrice() internal view returns (uint256 _hextherPrice) {
        try IOracle(hextherOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Hexther: failed to fetch HERMES price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _hextherPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_hextherPrice >= taxTiersTwaps[tierId]) {
    ...
    function setHextherOracle(address _hextherOracle) public onlyOperatorOrTaxOffice {
        require(_hextherOracle != address(0), "oracle address cannot be 0 address");
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentHextherPrice = _getHextherPrice();
            currentTaxRate = _updateTaxRate(currentHextherPrice);
            if (currentHextherPrice < burnThreshold) {
                burnTax = true;
            }
        }
